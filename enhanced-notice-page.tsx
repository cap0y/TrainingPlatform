...
export default function EnhancedNoticePage() {
  ...
  const categories = [
    ...
    { id: "notice", name: "공지사항", count: notices?.notices?.filter((n: Notice) => n.category === "notice").length || 0 },
    { id: "announcement", name: "안내", count: notices?.notices?.filter((n: Notice) => n.category === "announcement").length || 0 },
    { id: "update", name: "업데이트", count: notices?.notices?.filter((n: Notice) => n.category === "update").length || 0 },
    { id: "event", name: "이벤트", count: notices?.notices?.filter((n: Notice) => n.category === "event").length || 0 },
  ];
  ...
  const importantNotices = notices?.notices?.filter((notice: Notice) => notice.isImportant) || [];
              ...
              {selectedNotice.attachments && selectedNotice.attachments.length > 0 && (
                <>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedNotice.attachments.map((file: { name: string }, index: number) => (
