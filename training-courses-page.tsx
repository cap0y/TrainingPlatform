...
export default function TrainingCoursesPage() {
  ...
  const category = decodeURIComponent(searchParams.get('category') || '법정의무교육');
  const urlSubcategory = searchParams.get('subcategory') || 'all';
  
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedType, setSelectedType] = useState("all");
  const [selectedLevel, setSelectedLevel] = useState("all");
  const [selectedLegalCategory, setSelectedLegalCategory] = useState(urlSubcategory);
  ...
  const { data: coursesData, isLoading } = useQuery<CoursesResponse>({
    ...
    queryFn: async () => {
      ...
      if (queryParams.subcategory) searchParams.append('subcategory', queryParams.subcategory);
  ...
  const legalEducationCategories = [
    ...
    },
    {
      id: "화학물질",
      name: "화학물질 안전교육",
      count: coursesData?.courses.filter(c => c.title.includes("화학물질") || c.title.includes("MSDS")).length || 0,
    ...
    },
    {
      id: "산업안전",
      name: "산업안전보건교육",
      count: coursesData?.courses.filter(c => c.title.includes("산업안전") || c.title.includes("안전보건")).length || 0,
    ...
    },
    {
      id: "소방안전",
      name: "소방안전교육",
      count: coursesData?.courses.filter(c => c.title.includes("소방") || c.title.includes("화재")).length || 0,
    ...
    },
    {
      id: "환경안전",
      name: "환경안전교육",
      count: coursesData?.courses.filter(c => c.title.includes("환경") || c.title.includes("폐기물")).length || 0,
    ...
    },
    {
      id: "개인정보",
      name: "개인정보보호교육",
      count: coursesData?.courses.filter(c => c.title.includes("개인정보") || c.title.includes("정보보호")).length || 0,
    ...
    },
    {
      id: "성희롱예방",
      name: "성희롱예방교육",
      count: coursesData?.courses.filter(c => c.title.includes("성희롱") || c.title.includes("성폭력")).length || 0,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {category === '법정의무교육' ? (
          ...
          <div className="relative container mx-auto px-4 z-10">
            <div className="max-w-3xl">
              ...
              <p className="text-xl text-red-100 mb-6">
                법령에 의해 의무적으로 이수해야 하는 안전교육 및 보건교육 과정으로 
                안전한 작업환경을 만들어가세요.
              </p>
        ...
        <section className="relative bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16 overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 z-10">
            <div className="max-w-3xl">
              <h1 className="text-4xl font-bold mb-4">{getCategoryTitle(category)}</h1>
      ...
      {category === '법정의무교육' && (
        <section className="py-12 bg-white">
          <div className="container mx-auto px-4">
            <h2 className="text-2xl font-bold text-center mb-8 text-gray-800">법정의무교육 분야</h2>
                ...
                {legalEducationCategories.map((category) => (
                  <div 
                    ...
                    className={`text-center cursor-pointer group ${
                      selectedLegalCategory === category.id ? 'transform scale-105' : ''
                    }`}
                  ...
                  >
                    <div className={`relative w-20 h-20 mx-auto mb-3 overflow-hidden rounded-full shadow-lg group-hover:shadow-xl transition-shadow duration-300 ${
                      selectedLegalCategory === category.id ? 'ring-4 ring-red-500' : ''
                    }`}>
                      <img 
                    ...
                    <div className={`font-medium text-sm transition-colors ${
                      ...
                      {category.id === "all" ? category.name : category.name.replace("교육", "")}
                    </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="flex justify-center">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 max-w-4xl">
                {legalEducationCategories.filter(cat => cat.id !== "all").map((category) => (
                  <Link 
                    key={category.id}
                    ...
                    <div className="text-center cursor-pointer group">
                      <div className="relative w-20 h-20 mx-auto mb-3 overflow-hidden rounded-full shadow-lg group-hover:shadow-xl transition-shadow duration-300">
                        <img 
                          ...
                          className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                        />
                        <div className={`absolute inset-0 ${category.overlay} bg-opacity-20 group-hover:bg-opacity-10 transition-opacity duration-300`}></div>
                      </div>
      ...
      <main className="container mx-auto px-4 py-8">
            ...
            <Card className="p-6 bg-gradient-to-r from-red-50 to-red-100">
              <CardContent className="p-0">
                <div className="flex items-center mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-red-800">법정 의무사항</h3>
                </div>
            ...
            <Card className="p-6 bg-gradient-to-r from-blue-50 to-blue-100">
              <CardContent className="p-0">
                <div className="flex items-center mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-blue-800">수료증 발급</h3>
                </div>
            ...
            <Card className="p-6 bg-gradient-to-r from-green-50 to-green-100">
              <CardContent className="p-0">
                <div className="flex items-center mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-green-800">이수 기한</h3>
                </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">등록된 법정의무교육 과정이 없습니다</h3>
            <p className="text-gray-500">새로운 교육과정이 곧 추가될 예정입니다.</p>
          </div>
        )}
