...
export default function AuthPage() {
  const { user, loginMutation, registerMutation, kakaoLoginMutation, googleLoginMutation } = useAuth();
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 회원 유형 불일치 에러 처리
      if (error?.response?.status === 401 || error?.message?.includes('userType')) {
        toast({
          title: "로그인 실패",
          description: memberType === "business" 
            ? "기관회원 정보가 일치하지 않습니다. 개인회원이신가요?" 
            : "개인회원 정보가 일치하지 않습니다. 기관회원이신가요?",
          variant: "destructive",
        });
                      ...
                      <form onSubmit={handleLoginSubmit} className="space-y-6">
                        ...
                        {memberType === "business" && (
                          <div>
                            ...
                            <Input
                              id="businessNumber"
                              placeholder="000-00-00000 (하이픈 포함)"
                          ...
                          <>
                            ...
                            <div className="space-y-3">
                              ...
                              >
                                ...
                                {kakaoLoginMutation.isPending ? "카카오 로그인 중..." : "카카오로 로그인"}
                              </Button>
                              ...
                              >
                                ...
                                {googleLoginMutation.isPending ? "구글 로그인 중..." : "구글로 로그인"}
                              </Button>
                            </div>
                          </>
                        )}
