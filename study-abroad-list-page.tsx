...
import { 
  ...
  Search, Filter, ChevronRight, Plane, Heart
} from "lucide-react";
...
export default function StudyAbroadListPage() {
  ...
  const { data: programsData, isLoading } = useQuery<{ programs: OverseasProgram[]; total: number }>({
    queryKey: ["/api/overseas-programs", { search: searchQuery, category: selectedCategory, country: selectedCountry, level: selectedLevel }],
  });
  ...
  const categories = [
    ...
    { id: "교육시찰", name: "교육시찰", count: programs.filter(p => p.type === "교육시찰").length },
    { id: "연구연수", name: "연구연수", count: programs.filter(p => p.type === "연구연수").length },
    { id: "교육과정개발", name: "교육과정개발", count: programs.filter(p => p.type === "교육과정개발").length },
    { id: "국제교류", name: "국제교류", count: programs.filter(p => p.type === "국제교류").length },
    { id: "어학연수", name: "어학연수", count: programs.filter(p => p.type === "어학연수").length },
    { id: "문화체험", name: "문화체험", count: programs.filter(p => p.type === "문화체험").length }
  ];
  ...
  const filteredPrograms = programs.filter(program => {
    ...
    const matchesCategory = selectedCategory === "all" || program.type === selectedCategory;
    const matchesCountry = selectedCountry === "전체" || program.destination.includes(selectedCountry);
    
    return matchesSearch && matchesCategory && matchesCountry && program.status === "active" && program.approvalStatus === "approved";
  });
  ...
  const getCategoryImage = (categoryId: string) => {
    const imageMap: { [key: string]: string } = {
      ...
      "교육과정개발": "https://images.unsplash.com/photo-1513258496099-48168024aec0",
      ...
      <section className="relative bg-gradient-to-r from-blue-600 to-purple-800 text-white py-16 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            <h1 className="text-4xl font-bold mb-4">글로벌 해외연수 프로그램</h1>
            ...
            {categories.map((category) => (
              ...
              >
                ...
                <div className={`font-medium transition-colors ${
                  selectedCategory === category.id ? 'text-purple-600' : 'text-gray-800'
                }`}>
                  {category.name}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-gray-500">새로운 프로그램이 곧 추가될 예정입니다.</p>
              {selectedCategory !== "all" && (
                <Button 
                  variant="outline" 
                  className="mt-4"
