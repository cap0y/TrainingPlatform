...
import { Calendar, MapPin, Users, Star, Clock, DollarSign, Search, Heart } from "lucide-react";
...
export default function SeminarsPage() {
  ...
  const categories = [
    ...
    { id: "교육학회", name: "교육학회", count: seminars.filter(s => s.type === "교육학회").length },
    { id: "AI 컨퍼런스", name: "AI 컨퍼런스", count: seminars.filter(s => s.type === "AI 컨퍼런스").length },
    { id: "워크샵", name: "워크샵", count: seminars.filter(s => s.type === "워크샵").length },
    { id: "심포지엄", name: "심포지엄", count: seminars.filter(s => s.type === "심포지엄").length },
    { id: "국제행사", name: "국제행사", count: seminars.filter(s => s.type === "국제행사").length },
    { id: "온라인세미나", name: "온라인세미나", count: seminars.filter(s => s.type === "온라인세미나").length }
  ];
  ...
  const filteredSeminars = seminars.filter(seminar => {
    ...
    const matchesCategory = selectedCategory === "all" || seminar.type === selectedCategory;
  ...
  const getSeminarStatus = (seminar: Seminar) => {
    ...
    if (seminarDate < now) {
      return "마감";
    } else if (seminar.maxParticipants && seminar.currentParticipants && seminar.currentParticipants >= seminar.maxParticipants) {
      return "마감";
    } else {
      return "접수중";
    }
  ...
  const wishlistMutation = useMutation({
    mutationFn: async ({ seminarId, action }: { seminarId: number; action: 'add' | 'remove' }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {seminarsLoading ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">등록된 세미나가 없습니다</h3>
              <p className="text-gray-500">새로운 세미나가 곧 등록될 예정입니다.</p>
            </div>
          ) : (
            filteredSeminars.map((seminar) => (
              <Card key={seminar.id} className={`group hover:shadow-lg transition-all duration-300 overflow-hidden ${seminar.featured ? 'border-2 border-yellow-400' : ''}`}>
                <div className="relative">
                  <img src={getImageUrl(seminar)} alt={seminar.title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
                  {seminar.featured && (
                  ...
                  <div className="absolute top-3 right-3">
                    <Badge className={`${getStatusColor(getSeminarStatus(seminar))} text-white`}>
                ...
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-gray-600 mb-3 line-clamp-2">{seminar.description || "세미나에 대한 상세 정보를 확인해보세요."}</p>
        ...
        {/* Empty State */}
        {!seminarsLoading && filteredSeminars.length === 0 && seminars.length > 0 && (
