...
import { 
  ...
  TrendingUp, CheckCircle, Bell, Headphones, Clock
} from "lucide-react";
...
export default function HomePage() {
  ...
  const [selectedOverseasCategory, setSelectedOverseasCategory] = useState("전체");
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("전체");
  const [selectedEducationCategory, setSelectedEducationCategory] = useState("전체");
  ...
  const { data: seminarsData, isLoading: seminarsLoading } = useQuery<Seminar[]>({
    queryKey: ["/api/seminars"],
  });

  const { data: overseasData, isLoading: overseasLoading } = useQuery<{programs: OverseasProgram[], total: number}>({
    queryKey: ["/api/overseas-programs"],
  });

  const notices = noticesData?.notices || [];
  const seminars = seminarsData || [];
  const overseas = Array.isArray(overseasData?.programs) ? overseasData.programs : [];
  ...
  const filteredOverseas = selectedOverseasCategory === "전체" 
    ? overseas 
    : (Array.isArray(overseas) ? overseas.filter(program => program.type === selectedOverseasCategory) : []);

  // Filter courses for different tabs
  const getPopularCourses = () => {
    // 실시간 인기: 등록자 수와 평점을 기준으로 정렬
    return courses
      .filter(course => course.status === "active" && course.approvalStatus === "approved")
      .sort((a, b) => {
        const scoreA = (a.enrolledCount || 0) * 0.7 + (a.rating || 0) * 0.3;
        const scoreB = (b.enrolledCount || 0) * 0.7 + (b.rating || 0) * 0.3;
  ...
  const getCategoryCourses = () => {
    ...
    const categories = ["교육과정", "자격증", "전문성강화", "교육평가", "교수법", "융합교육"];
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="relative z-20 container mx-auto px-4 py-12 pt-[80px] pb-[80px]">
          <div className="max-w-2xl text-left text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/courses">
                <Button size="lg" className="bg-white text-blue-600 hover:bg-gray-100 px-6 py-3 text-base font-semibold">
                  <BookOpen className="h-4 w-4 mr-2" />
                  연수과정 보기
                </Button>
              </Link>
              <Link href="/seminars">
                <Button size="lg" variant="outline" className="border-white text-white bg-white/20 px-6 py-3 text-base font-semibold">
                  세미나 신청
                </Button>
              </Link>
            </div>
      ...
      <section className="py-2 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-bold text-center mb-4 text-gray-800">교육 분야</h2>
          ...
          <div className="flex lg:hidden overflow-x-auto space-x-6 pb-4 px-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <Link href="/training-courses?category=법정의무교육">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">법정교육</div>
              </div>
            </Link>
            ...
            <Link href="/professional-development">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">전문성강화</div>
              </div>
            </Link>
            ...
            <Link href="/certificate-courses">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">자격증</div>
              </div>
            </Link>
            ...
            <Link href="/seminars">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">세미나</div>
              </div>
            </Link>
            ...
            <Link href="/study-abroad">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">해외연수</div>
              </div>
            </Link>
            ...
            <Link href="/help">
              <div className="flex-shrink-0 text-center cursor-pointer">
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">고객센터</div>
              </div>
            </Link>
          </div>
      ...
      <section className="py-8 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">인기 연수과정</h2>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="popular" className="space-y-6">
              <div className="overflow-x-auto pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <div className="flex space-x-4 animate-scroll-left hover:animate-none w-max">
                  {coursesLoading ? (
                    Array.from({ length: 10 }, (_, i) => (
                      <Card key={i} className="flex-shrink-0 w-80 h-[480px] flex flex-col overflow-hidden">
                    ...
                    [...popularCourses, ...popularCourses].map((course, index) => (
                      <Card key={`${course.id}-${index}`} className="group hover:shadow-lg transition-all duration-300 overflow-hidden flex-shrink-0 w-80 h-[430px] flex flex-col">
                        <div className="relative">
                          <img 
                            src={course.imageUrl && course.imageUrl !== "/api/placeholder/400/250" 
                              ? course.imageUrl 
                          ...
                          <div className="absolute top-2 right-2">
                            <Badge variant="outline" className="bg-white/90 text-xs">{course.category}</Badge>
                          </div>
                        ...
                        <CardContent className="p-4 flex flex-col flex-grow">
                          <h3 className="font-semibold text-base mb-3 group-hover:text-blue-600 transition-colors line-clamp-2">{course.title}</h3>
                          ...
                          <div className="space-y-2 mb-4">
                            ...
                            <div className="flex items-center text-xs text-gray-600">
                              ...
                              <span>난이도: {course.level === 'beginner' ? '기초' : 
                                           course.level === 'intermediate' ? '중급' : 
                                           course.level === 'advanced' ? '고급' : '중급'}</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-600 mb-4">
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <span>/{course.enrolledCount || 145}명 수강중</span>
                            </div>
                          </div>
                            ...
                            {course.discountPrice ? (
                              <div className="flex items-center space-x-2">
                                <span className="text-lg font-bold text-blue-600">
                                  {new Intl.NumberFormat("ko-KR").format(course.discountPrice)}원
                                </span>
                                <span className="text-sm text-gray-500 line-through">
                                  {new Intl.NumberFormat("ko-KR").format(course.price)}원
                                </span>
                              </div>
                            ) : (
                              <span className="text-lg font-bold text-blue-600">
                          ...
                          <div className="mt-1">
                            <Link href={`/courses/${course.id}`}>
                              <Button className="w-full text-sm py-2">수강신청</Button>
                            </Link>
                          </div>
                        </CardContent>
                      </Card>
                ...
                <div className="flex space-x-4 animate-scroll-left hover:animate-none w-max">
                  {coursesLoading ? (
                    ...
                    [...categoryCourses, ...categoryCourses].map((course, index) => (
                      <Card key={`${course.id}-${index}`} className="group hover:shadow-lg transition-all duration-300 overflow-hidden flex-shrink-0 w-80 h-[480px] flex flex-col">
                        <div className="relative">
                          <img 
                            src={course.imageUrl && course.imageUrl !== "/api/placeholder/400/250" 
                              ? course.imageUrl 
      ...
      <section className="py-8 bg-white">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">다가오는 학회 및 세미나</h2>
          ...
          <div className="mb-4">
            ...
            <div className="flex lg:hidden overflow-x-auto space-x-4 pb-4 px-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">전체</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">교육학회</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">AI 컨퍼런스</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">워크샵</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">심포지엄</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">국제행사</div>
              </div>
              ...
              >
                ...
                <div className="font-medium text-gray-800 text-sm whitespace-nowrap">온라인세미나</div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-4">
            {filteredSeminars.slice(0, 3).map((seminar) => (
              <Card key={seminar.id} className="group hover:shadow-lg transition-all duration-300 overflow-hidden">
                <div className="relative">
                  <img 
                    src={seminar.imageUrl && seminar.imageUrl !== "/api/placeholder/400/250" 
                      ? seminar.imageUrl 
                      : "/uploads/images/5.jpg"} 
                    alt={seminar.title} 
                ...
                <CardContent className="p-4">
                  <h3 className="font-semibold text-lg mb-3 group-hover:text-blue-600 transition-colors">{seminar.title}</h3>
                  <div className="space-y-2 text-sm text-gray-600 mb-4">
                    <div className="flex items-center space-x-2">
                      <Calendar className="h-4 w-4" />
                      <span>{new Date(seminar.date).toLocaleDateString('ko-KR', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        weekday: 'short'
      ...
      <section className="py-8 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">해외연수 프로그램</h2>
              <p className="text-gray-600">글로벌 역량 강화를 위한 다양한 해외연수 기회</p>
            </div>
            <Link href="/study-abroad">
              <Button size="lg" variant="outline">모든 해외연수 보기</Button>
            </Link>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex lg:hidden overflow-x-auto space-x-4 pb-4 px-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  ...
                  {selectedOverseasCategory === "전체" && <div className="absolute inset-0 ring-2 ring-green-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">전체</div>
              </div>

              <div 
                className={`flex-shrink-0 text-center cursor-pointer ${selectedOverseasCategory === "학술연수" ? "opacity-100" : "opacity-60"}`}
                onClick={() => setSelectedOverseasCategory("학술연수")}
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  <img 
                  ...
                  />
                  <div className="absolute inset-0 bg-blue-600 bg-opacity-10"></div>
                  {selectedOverseasCategory === "학술연수" && <div className="absolute inset-0 ring-2 ring-blue-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">학술연수</div>
              </div>

              <div 
                className={`flex-shrink-0 text-center cursor-pointer ${selectedOverseasCategory === "문화체험" ? "opacity-100" : "opacity-60"}`}
              ...
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  ...
                  />
                  <div className="absolute inset-0 bg-purple-600 bg-opacity-10"></div>
                  {selectedOverseasCategory === "문화체험" && <div className="absolute inset-0 ring-2 ring-purple-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">문화체험</div>
              </div>

              <div 
                className={`flex-shrink-0 text-center cursor-pointer ${selectedOverseasCategory === "어학연수" ? "opacity-100" : "opacity-60"}`}
              ...
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  ...
                  />
                  <div className="absolute inset-0 bg-green-600 bg-opacity-10"></div>
                  {selectedOverseasCategory === "어학연수" && <div className="absolute inset-0 ring-2 ring-green-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">어학연수</div>
              </div>

              <div 
                className={`flex-shrink-0 text-center cursor-pointer ${selectedOverseasCategory === "현장학습" ? "opacity-100" : "opacity-60"}`}
                onClick={() => setSelectedOverseasCategory("현장학습")}
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  <img 
                  ...
                  />
                  <div className="absolute inset-0 bg-orange-600 bg-opacity-10"></div>
                  {selectedOverseasCategory === "현장학습" && <div className="absolute inset-0 ring-2 ring-orange-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">현장학습</div>
              </div>

              <div 
                className={`flex-shrink-0 text-center cursor-pointer ${selectedOverseasCategory === "국제회의" ? "opacity-100" : "opacity-60"}`}
                onClick={() => setSelectedOverseasCategory("국제회의")}
              >
                <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                  <img 
                  ...
                  />
                  <div className="absolute inset-0 bg-red-600 bg-opacity-10"></div>
                  {selectedOverseasCategory === "국제회의" && <div className="absolute inset-0 ring-2 ring-red-500 rounded-full"></div>}
                </div>
                <div className="font-medium text-gray-800 text-xs whitespace-nowrap">국제회의</div>
              </div>
            </div>
            ...
            <div className="hidden lg:flex lg:justify-center">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl">
                ...
                <div 
                  className={`text-center cursor-pointer ${selectedOverseasCategory === "학술연수" ? "opacity-100" : "opacity-60"}`}
                  onClick={() => setSelectedOverseasCategory("학술연수")}
                >
                  <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                    <img 
                  ...
                  <div className="font-medium text-gray-800 text-xs">학술연수</div>
                </div>
                ...
                >
                  ...
                  <div className="font-medium text-gray-800 text-xs">문화체험</div>
                </div>
                ...
                >
                  ...
                  <div className="font-medium text-gray-800 text-xs">어학연수</div>
                </div>

                <div 
                  className={`text-center cursor-pointer ${selectedOverseasCategory === "현장학습" ? "opacity-100" : "opacity-60"}`}
                  onClick={() => setSelectedOverseasCategory("현장학습")}
                >
                  <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                    <img 
                  ...
                  <div className="font-medium text-gray-800 text-xs">현장학습</div>
                </div>

                <div 
                  className={`text-center cursor-pointer ${selectedOverseasCategory === "국제회의" ? "opacity-100" : "opacity-60"}`}
                  onClick={() => setSelectedOverseasCategory("국제회의")}
                >
                  <div className="relative w-14 h-14 mx-auto mb-2 overflow-hidden rounded-full shadow-lg">
                    <img 
                  ...
                  <div className="font-medium text-gray-800 text-xs">국제회의</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {filteredOverseas.slice(0, 3).map((program) => (
              <Card key={program.id} className="group hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col">
                <div className="relative">
                  <img 
                    src={program.imageUrl && program.imageUrl !== "/api/placeholder/400/250" 
                      ? program.imageUrl 
                      : "/uploads/images/8.jpg"} 
                    alt={program.title} 
                    className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300"
                    onError={(e) => {
                      const fallbackImages = ["/uploads/images/8.jpg", "/uploads/images/9.jpg", "/uploads/images/10.jpg"];
                      const randomFallback = fallbackImages[Math.floor(Math.random() * fallbackImages.length)];
                      e.currentTarget.src = randomFallback;
                    }}
                  />
                  ...
                  <div className="absolute top-3 left-3">
                    <Badge className="bg-green-500 text-white">
                      {program.price ? `${program.price.toLocaleString()}원` : "문의"}
                    </Badge>
                  </div>
                </div>
                <CardContent className="p-4 flex flex-col flex-grow">
                  <h3 className="font-semibold text-lg mb-3 group-hover:text-blue-600 transition-colors">{program.title}</h3>
                  <div className="space-y-2 text-sm text-gray-600 mb-4 flex-grow">
                    ...
                    <div className="flex items-center space-x-2">
                      <Calendar className="h-4 w-4" />
                      <span>
                        {new Date(program.startDate).toLocaleDateString('ko-KR')} ~ {new Date(program.endDate).toLocaleDateString('ko-KR')}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Users className="h-4 w-4" />
                      <span>{program.maxParticipants || 30}명 모집 ({program.currentParticipants || 0}명 신청)</span>
                    </div>
                    {program.duration && (
                      <div className="flex items-center space-x-2">
      ...
      <section className="py-8 bg-white pt-[0px] pb-[0px]">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">공지사항</h2>
                  ...
                  <Link key={notice.id} href="/announcements">
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                      <div className="flex items-center space-x-3">
                        ...
                        {notice.isImportant && (
                          <Badge className="bg-red-500 text-white text-xs">중요</Badge>
                        )}
                      </div>
                      <div className="flex items-center space-x-2">
                        ...
                        {notice.views && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-xs text-gray-400">{notice.views}</span>
                          </div>
                        )}
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-3 p-3 bg-white rounded-lg">
                  ...
                  <div>
                    ...
                    <div className="font-semibold text-gray-900">055-772-2226</div>
                  </div>
                </div>
                ...
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white" onClick={() => setIsChatOpen(true)}>
                  <MessageCircle className="h-4 w-4 mr-2" />
                  실시간 상담 시작하기
                </Button>

                <div className="mt-3">
                  <Link href="/enhanced-help">
