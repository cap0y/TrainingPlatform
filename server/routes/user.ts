...
export function registerUserRoutes(app: Express) {
  ...
  // 특정 강의 수강 정보 조회 API
  app.get("/api/user/enrollments/course/:courseId", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "수강 정보 조회 중 오류가 발생했습니다." });
    }
  });

  // 수료증 발급 API
  app.post("/api/user/enrollments/:enrollmentId/issue-certificate", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!course) {
        return res.status(404).json({ message: "과정 정보를 찾을 수 없습니다." });
      }

      // 진도율 80% 이상인지 확인
      if (!enrollment.progress || enrollment.progress < 80) {
        return res.status(400).json({ message: "진도율이 80% 이상일 때 수료증을 발급받을 수 있습니다." });
      }

      // 이미 발급된 수료증이 있는지 확인
      const existingCertificate = await storage.getCertificate(enrollmentId);
      if (existingCertificate) {
        return res.status(400).json({ message: "이미 발급된 수료증이 있습니다." });
      }
      ...
      res.status(500).json({ message: "수료증 발급 중 오류가 발생했습니다." });
    }
  });

  // 수료증 조회 API
  app.get("/api/user/enrollments/:enrollmentId/certificate", isAuthenticated, async (req, res) => {
    try {
      const enrollmentId = parseInt(req.params.enrollmentId);
      const userId = req.user?.id;
      ...
      res.status(500).json({ message: "수료증 생성 중 오류가 발생했습니다." });
    }
  });
  ...
  async function ensureProgressTable(requestId: string) {
    ...
    if (progressTableInitializing) {
      let attempts = 0;
      while (progressTableInitializing && attempts < 50) { // 최대 5초 대기
  ...
  // 전체 진도율 계산 헬퍼 함수
  async function calculateTotalProgress(enrollmentId: number, userId: number, course: any): Promise<number> {
    try {
      // 테이블 존재 여부 확인
      const tableExists = await storage.query(`
        SELECT EXISTS (
      ...
      for (const item of course.curriculumItems) {
        ...
        if (item.videos && Array.isArray(item.videos) && item.videos.length > 0) {
          ...
          for (const video of item.videos) {
            try {
              ...
              if (videoResult.rows.length > 0 && videoResult.rows[0].progress >= 90) {
                completedItems++;
              }
            } catch (error) {
        ...
        // 퀴즈 항목 처리
        if (item.quizzes && Array.isArray(item.quizzes) && item.quizzes.length > 0) {
          ...
          for (const quiz of item.quizzes) {
            try {
              ...
              if (quizResult.rows.length > 0 && quizResult.rows[0].progress >= 60) {
                completedItems++;
              }
            } catch (error) {
  ...
  // 진도율 업데이트 API
  app.post("/api/user/enrollments/:enrollmentId/progress", isAuthenticated, async (req, res) => {
    try {
      const enrollmentId = parseInt(req.params.enrollmentId);
      ...
      res.status(500).json({ message: "진도율 업데이트 중 오류가 발생했습니다." });
    }
  });

  // 진도율 조회 API
  app.get("/api/user/enrollments/:enrollmentId/progress", isAuthenticated, async (req, res) => {
    try {
      const enrollmentId = parseInt(req.params.enrollmentId);
      const userId = req.user?.id;
      ...
      res.status(500).json({ message: "진도율 조회 중 오류가 발생했습니다." });
    }
  });
} 