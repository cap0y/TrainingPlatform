...
import { BookOpen, Calendar, CreditCard, User, Settings, Award, Clock, CheckCircle, Video, Plane, Eye, Download, Globe } from "lucide-react";
...
export default function MyPage() {
  ...
  const { data: overseasRegistrations, isLoading: overseasLoading } = useQuery<OverseasRegistration[]>({
  ...
  const downloadCertificate = async (enrollmentId: number, courseTitle: string) => {
    try {
      setDownloadingCertificate(enrollmentId.toString());
      
      const response = await fetch(`/api/user/enrollments/${enrollmentId}/certificate/download`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      toast({
        title: "다운로드 실패",
        description: error instanceof Error ? error.message : "수료증 다운로드 중 오류가 발생했습니다.",
        variant: "destructive",
      });
    } finally {
  ...
  const calculateStats = () => {
    ...
    const inProgress = enrollmentList.filter(e => e.status === 'enrolled').length;
              ...
              <CardContent>
                {enrollmentsLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">수강 중인 과정이 없습니다</h3>
                    <p className="text-sm text-gray-500 mb-4">새로운 과정을 둘러보고 학습을 시작해보세요.</p>
                    <Button asChild>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {enrollments?.map((enrollment) => (
                      <div key={enrollment.id} className="border rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex flex-col h-full">
                          ...
                          <div className="flex items-center mb-4">
                            <Badge variant={getStatusBadge(enrollment.status)}>
                              {enrollment.status === 'enrolled' ? '수강중' : 
                               ...
                               enrollment.status === 'cancelled' ? '취소됨' : '대기중'}
                            </Badge>
                          </div>
                          ...
                          <div className="mt-auto">
                            <div className="text-sm text-gray-500 mb-3">
                              신청일: {enrollment.enrolledAt ? new Date(enrollment.enrolledAt).toLocaleDateString('ko-KR', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                              }) : '날짜 없음'}
                ...
                {seminarsLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {seminars?.map((seminar) => (
                      <Card key={seminar.id} className="border-2 border-dashed border-gray-200 hover:border-primary transition-colors">
                        <CardContent className="p-6">
                          <div className="flex justify-between items-start mb-4">
                            <Video className="h-8 w-8 text-primary" />
                            {getSeminarStatusBadge(seminar.status)}
                          </div>
                          <h3 className="font-semibold mb-2">{seminar.title}</h3>
                          <div className="space-y-2 mb-4 text-sm text-gray-600">
                            <p>일시: {new Date(seminar.date).toLocaleString('ko-KR')}</p>
                            <p>장소: {seminar.location}</p>
                            {seminar.speaker && <p>강연자: {seminar.speaker}</p>}
                          </div>
                          <div className="flex justify-end">
                            {seminar.status === 'registered' && (
                              ...
                              >
                                <a 
                                  href={seminar.link || `https://meet.google.com/seminar/${seminar.id}`} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                >
                                  세미나 참여하기
                            ...
                            {seminar.status === 'completed' && (
                              ...
                              >
                                <a 
                                  href={seminar.recordingLink || `https://video.example.com/recording/${seminar.id}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                >
                                  다시보기
                ...
                {overseasLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <Button className="mt-4" asChild>
                      <Link href="/study-abroad">해외연수 프로그램 둘러보기</Link>
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {overseasRegistrations?.map((registration) => (
                      <Card key={registration.id} className="hover:bg-gray-50 transition-colors">
                        <CardContent className="p-6">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <h3 className="font-semibold text-lg">{registration.overseas_program?.title}</h3>
                            ...
                            <Badge 
                              className={
                                registration.status === 'registered' ? 'bg-blue-500' :
                                registration.status === 'completed' ? 'bg-green-500' :
                                'bg-red-500'
                              }
                            >
                              {
                                registration.status === 'registered' ? '신청완료' :
                                registration.status === 'completed' ? '연수완료' :
                                '취소됨'
                              }
                            </Badge>
                          </div>
                          ...
                          <div className="space-y-2 mb-4 text-sm text-gray-600">
                            <p className="flex items-center">
                              ...
                              연수 기간: {registration.overseas_program?.startDate && new Date(registration.overseas_program.startDate).toLocaleDateString('ko-KR')} ~ {registration.overseas_program?.endDate && new Date(registration.overseas_program.endDate).toLocaleDateString('ko-KR')}
                            </p>
                            ...
                            <p className="flex items-center">
                              ...
                              신청일: {registration.registered_at && new Date(registration.registered_at).toLocaleDateString('ko-KR')}
                            </p>
                          </div>
                          
                          <div className="flex justify-end">
                            {registration.status === 'registered' && registration.overseas_program?.id && (
                              <Button size="sm" asChild>
                                <Link href={`/study-abroad/${registration.overseas_program.id}`}>
                                  상세 정보
                                </Link>
                              </Button>
                            )}
              ...
              <CardContent>
                {enrollmentsLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">아직 수료한 과정이 없습니다.</p>
                    <Button className="mt-4" asChild>
                      <Link href="/courses">과정 둘러보기</Link>
                    </Button>
                  </div>
                      ...
                      <Card key={enrollment.id} className="border-2 border-dashed border-gray-200 hover:border-primary transition-colors">
                        <CardContent className="p-6">
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <Badge variant="outline">{enrollment.course?.credit}학점</Badge>
                          </div>
                          <h3 className="font-semibold mb-2 line-clamp-2">{enrollment.course?.title}</h3>
                          <div className="space-y-2 mb-4 text-sm text-gray-600">
                            <p>수료일: {new Date(enrollment.enrolledAt).toLocaleDateString('ko-KR')}</p>
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                            <div>
                              ...
                              <div className="space-y-1">
                                <p>결제 방식: {payment.paymentMethod || '카드 결제'}</p>
