...
import { Calendar, MapPin, Users, Clock, DollarSign, Star, Share2, Heart, ArrowLeft, Loader2, Copy, Facebook, Twitter, MessageCircle, Plane, Globe, Home, Utensils, UserCheck } from "lucide-react";
...
export default function StudyAbroadDetailPage() {
  ...
  const { data: program, isLoading, error } = useQuery<OverseasProgram>({
  ...
  const registerMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`/api/overseas-programs/${programId}/register`, {
        method: 'POST',
        headers: {
    ...
    onSuccess: (data) => {
      alert(data.message);
      queryClient.invalidateQueries({ queryKey: [`/api/overseas-programs/${programId}`] });
    },
    onError: (error: Error) => {
      alert(error.message);
    },
  ...
  if (error || !program) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header />
        <div className="container mx-auto px-4 py-16 text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">해외연수를 찾을 수 없습니다</h1>
          <p className="text-gray-600 mb-8">요청하신 해외연수가 존재하지 않거나 삭제되었습니다.</p>
  ...
  // 기본값 설정
  const defaultImageUrl = "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=800&h=400&fit=crop";
  ...
  const getStatusColor = (isActive: boolean) => {
    ...
    if (startDate < now) {
      return "bg-red-500";
    } else if (program.maxParticipants && program.currentParticipants && program.currentParticipants >= program.maxParticipants) {
      return "bg-red-500";
    } else {
      return "bg-green-500";
    }
            ...
            <div className="container mx-auto px-4">
              <div className="max-w-4xl text-white">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <Badge className={`${getStatusColor(program.isActive || false)} text-white`}>
                    {getStatusText()}
                  </Badge>
                  ...
                  <div className="space-y-6">
                    {program.programSchedule.map((schedule, index) => (
                      <div key={schedule.id} className="relative">
                        ...
                        <div className={`relative pl-12 pr-4 py-4 rounded-lg border-l-4 ${getTypeColor(schedule.type)}`}>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {program.visaInfo && (
                    <div>
                      <h3 className="font-semibold text-gray-800 mb-2">비자 정보</h3>
                      <p className="text-gray-600">{program.visaInfo}</p>
                    </div>
                  )}
                  {program.currency && (
                    <div>
                      <h3 className="font-semibold text-gray-800 mb-2">현지 통화</h3>
                  ...
                  {program.language && (
                    <div>
                      <h3 className="font-semibold text-gray-800 mb-2">사용 언어</h3>
                      <p className="text-gray-600">{program.language}</p>
                    </div>
                  )}
                  {program.emergencyContact && (
                    <div>
                      <h3 className="font-semibold text-gray-800 mb-2">긴급 연락처</h3>
                      <p className="text-gray-600">{program.emergencyContact}</p>
                    </div>
                  )}
                </div>
              </CardContent>
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold">
                        {program.currentParticipants || 0}/{program.maxParticipants}명
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg max-w-md w-full mx-4">
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <Button className="flex-1 bg-purple-600 hover:bg-purple-700" onClick={handlePaymentComplete}>
